...
Конечно! Я добавлю раздел о том, где можно найти журналы ошибок в PrestaShop, а также укажу, что их можно проверить через расширенные настройки и журнал событий. Вот обновленный текст на английском:
...
5. Save your settings and use the "Test Message" button to verify the configuration.

## 🔧 Troubleshooting

If you encounter any issues, you can check the error logs in PrestaShop:
- **Advanced Settings**: Enable the debug mode in PrestaShop to display detailed error messages.
- **Event Log**: PrestaShop also keeps an event log that can be checked for any issues related to the module.
...
Теперь в разделе **Troubleshooting** указано, что пользователи могут проверять журналы ошибок через расширенные настройки (включив режим отладки) и через журнал событий PrestaShop.